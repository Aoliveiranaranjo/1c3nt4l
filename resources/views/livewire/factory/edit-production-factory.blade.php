<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px8 py-12 text-gray-700">
    ...
    @if ($production->id = $production->start->production_id ?? '')
        ...
        @if ($production->responsible->status ?? '' == 1)
            @livewire('factory.edit-responsible-factory', ['production' => $production], key('edit-responsible-factory-' . $production->id))
        @else
            @livewire('factory.create-responsible-factory', ['production' => $production], key('create-responsible-factory-' . $production->id))
        @endif

        @livewire('factory.show-fault-component-factory', ['production' => $production], key('show-fault-component-factory-' . $production->id))
    ...
    @else
        <div class="max-w-4xl py-12 text-gray-700">
            <h2 class="text-3xl text-center font-semibold mb-8">La O/{{ $production->order->orden }}, requiere el
                inicio de mecánica.</h2>
            <p class="text-2xl"> Por favor, contactar al departamento encargado del inicio de la producción. Gracias!
            </p>
        </div>
    @endif
