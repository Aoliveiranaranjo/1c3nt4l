<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px8 py-12 text-gray-700">
    ...
    <h2 class="text-3xl text-center font-semibold mb-8">Inicio de Calidad. Orden: O/ {{ $production->order->orden }} </h2>
    ...
    @livewire('factory.show-quality-one', ['production' => $production], key('show-show-quality-one-' . $production->id))


    @else

    @livewire('factory.create-quality-factory', ['production' => $production], key('create-quality-factory-' . $production->id))
